...
# Coinglass API Key 우선순위: secrets.toml > 환경변수 > 상수
DEFAULT_COINGLASS_API_KEY = "8cca09baf8cc497dbb5a4caba45a41f6"

def _get_secret(name: str, default: str = "") -> str:
    """Streamlit secrets가 없을 때도 안전하게 읽기."""
    # 1) secrets.toml 시도 (없으면 예외를 삼켜서 넘어감)
    try:
        return st.secrets.get(name, default)  # 파일이 없으면 예외 발생 → except로 이동
    except Exception:
        # 2) 환경변수
...
def classify_core(df_raw: pd.DataFrame, config: AppConfig, progress_placeholder=None) -> Dict:
    ...
    for i, (_, r) in enumerate(df_raw.iterrows(), 1):
        ...
        rows.append({
            ...
            "Qty": qty_raw,
            "Price Close (USD on Trade Date)": price_close,
            "Qty USD (on Trade Date)": qty_usd_trade,
...
st.caption("TLS 인증서 자동 설정, 코드 모듈화, 기능 유지")
